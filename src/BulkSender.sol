...
contract BulkSender is Ownable {
    ...
    constructor(address protocolBeneficiary_, uint256 feePerRecipient_) Ownable(_msgSender()) {
     ...
     */
    function updateProtocolBeneficiary(address protocolBeneficiary_) external onlyOwner {
     ...
     */
    function updateFeePerRecipient(uint256 feePerRecipient_) external onlyOwner {
        feePerRecipient = feePerRecipient_;
    ...
    function _validateParams(address[] calldata recipients, uint256[] calldata amounts)
        private
        pure
    ...
    function _validateFees(uint256 recipientsCount) private view returns (uint256 totalFee) {
    ...
    function _collectFee(uint256 totalFee) private {
        if (totalFee > 0) {
            (bool success,) = payable(protocolBeneficiary).call{value: totalFee}("");
    ...
    function sendERC20(address token, address[] calldata recipients, uint256[] calldata amounts) external payable {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < recipientsCount; ++i) {
                IERC20(token).transferFrom(msgSender, recipients[i], amounts[i]);
            }
        } // gas optimization
    ...
    function sendERC1155(address token, address[] calldata recipients, uint256[] calldata amounts) external payable {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < recipientsCount; ++i) {
                IERC1155(token).safeTransferFrom(msgSender, recipients[i], 0, amounts[i], "");
            }
        } // gas optimization
