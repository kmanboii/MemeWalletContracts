 ...
 */
contract MCV2_Bond is Initializable, MCV2_Royalty, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    error MCV2_Bond__WrongPaymentToken();
    error MCV2_Bond__WrongPermitToken();
    error MCV2_Bond__WrongPermitAmount();
     ...
     */
    address private TOKEN_IMPLEMENTATION;
    ...
    event MultiTokenCreated(
        address indexed token, string name, string symbol, string uri, address indexed reserveToken
    );
    event Mint(
     ...
     * @param creationFee_ The creation fee for creating a token in payment token.
     * @param maxSteps_ The maximum number of steps allowed in a bond.
    ...
    function initialize(
        ...
        if (tokenImplementation_ == address(0)) revert MCV2_Bond__InvalidConstructorParams("tokenImplementation");
        if (protocolBeneficiary_ == address(0)) revert MCV2_Bond__InvalidConstructorParams("protocolBeneficiary");
        if (maxSteps_ == 0) revert MCV2_Bond__InvalidConstructorParams("maxSteps");
        if (permit2_ == address(0)) revert MCV2_Bond__InvalidConstructorParams("permit2");
        if (paymentToken_ == address(0)) revert MCV2_Bond__InvalidConstructorParams("paymentToken");
        if (initialOwner_ == address(0)) revert MCV2_Bond__InvalidConstructorParams("initialOwner");

        TOKEN_IMPLEMENTATION = tokenImplementation_;
    ...
    /**
     * @dev Reinitializes the reentrancy and pausable guards. Called when upgrading the contract
     */
     ...
     */
    function _validateMultiTokenParams(MultiTokenParams calldata tp) private pure {
        if (bytes(tp.name).length == 0) revert MCV2_Bond__InvalidTokenCreationParams("name");
        if (bytes(tp.symbol).length == 0) revert MCV2_Bond__InvalidTokenCreationParams("symbol");
     ...
     */
    function _checkMethodExists(address implementation, string memory method, uint256 minLength)
        private
        view
        returns (bool)
    {
    ...
    function _validateBondParams(BondParams calldata bp) private view {
        if (bp.reserveToken != PAYMENT_TOKEN) revert MCV2_Bond__WrongPaymentToken();
        if (bp.mintRoyalty > MAX_ROYALTY_RANGE) revert MCV2_Bond__InvalidTokenCreationParams("mintRoyalty");
        if (bp.burnRoyalty > MAX_ROYALTY_RANGE) revert MCV2_Bond__InvalidTokenCreationParams("burnRoyalty");
    ...
    function _setBond(address token, BondParams calldata bp) private {
        ...
        for (uint256 i = 0; i < bp.stepRanges.length; ++i) {
            ...
            bond.steps.push(BondStep({rangeTo: uint128(stepRange), price: uint128(stepPrice)}));
        }
    }
    ...
    function createToken(
        ...
        if (creationFee > 0) {
            if (permit.permitted.token != PAYMENT_TOKEN) revert MCV2_Bond__WrongPermitToken();
            if (permit.permitted.amount != creationFee) revert MCV2_Bond__WrongPermitAmount();

            ISignatureTransfer.SignatureTransferDetails memory transferDetails =
                ISignatureTransfer.SignatureTransferDetails({to: protocolBeneficiary, requestedAmount: creationFee});
            PERMIT2.permitTransferFrom(permit, transferDetails, _msgSender(), signature);
        }
     ...
     * @param permit The permit.
     * @param signature The permit signature.
     * @return The amount of reserve tokens used for the minting operation.
     */
    function mint(
        address token,
        ...
        if (permit.permitted.token != PAYMENT_TOKEN) revert MCV2_Bond__WrongPermitToken();
        if (permit.permitted.amount < reserveAmount) revert MCV2_Bond__WrongPermitAmount();

        // Create transfer details
        ISignatureTransfer.SignatureTransferDetails memory transferDetails =
            ISignatureTransfer.SignatureTransferDetails({to: address(this), requestedAmount: reserveAmount});
     ...
     * @param permit The permit.
     * @param signature The permit signature.
     * @return The amount of reserve tokens returned to the user.
     */
    function burn(
        address token,
        ...
        if (permit.permitted.token != token) revert MCV2_Bond__WrongPermitToken();
        if (permit.permitted.amount != tokensToBurn) revert MCV2_Bond__WrongPermitAmount();

        // Transfer tokens from user to the bond contract
        ISignatureTransfer.SignatureTransferDetails memory transferDetails =
            ISignatureTransfer.SignatureTransferDetails({to: address(this), requestedAmount: tokensToBurn});
        PERMIT2.permitTransferFrom(permit, transferDetails, user, signature);

        // Burn tokens from the bond contract
        MCV2_ICommonToken(token).burnByBond(address(this), tokensToBurn);
    ...
    /**
     * @dev Transfers tokens with a permit signature.
     * @param token The address of the token to transfer.
     * @param permit The permit.
     * @param signature The permit signature
     * @param sendAmount The amount of tokens to transfer.
    ...
    function transferWithPermit(
        ...
        uint256 sendAmount,
        address recipient
    ) external whenNotPaused nonReentrant _checkBondExists(token) {
        address user = _msgSender();
        if (permit.permitted.token != token) revert MCV2_Bond__WrongPermitToken();
        if (permit.permitted.amount != sendAmount) revert MCV2_Bond__WrongPermitToken();
        ISignatureTransfer.SignatureTransferDetails memory transferDetails =
            ISignatureTransfer.SignatureTransferDetails({to: recipient, requestedAmount: sendAmount});
        PERMIT2.permitTransferFrom(permit, transferDetails, user, signature);
    }
     ...
     */
    function getTokensByReserveToken(address reserveToken, uint256 start, uint256 stop)
        external
        view
     ...
     */
    function getTokensByCreator(address creator, uint256 start, uint256 stop)
        external
        view
        returns (address[] memory addresses)
    {
    ...
    function updateMaxSteps(uint256 maxSteps) external onlyOwner {
        MAX_STEPS = maxSteps;
    }
}
