...
contract MerkleDistributorV2 is Ownable {
    ...
    constructor(address protocolBeneficiary_, uint256 claimFee_) Ownable(msg.sender) {
     ...
     */
    function claim(uint256 distributionId, bytes32[] calldata merkleProof) external payable {
        Distribution storage distribution = distributions[distributionId];
        ...
        if (distribution.merkleRoot == bytes32(0)) {
            ...
            if (!MerkleProof.verify(merkleProof, distribution.merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
        ...
        if (distribution.merkleRoot == bytes32(0) && claimFee > 0) {
            (bool success,) = payable(protocolBeneficiary).call{value: claimFee}("");
            if (!success) {
     ...
     */
    function refund(uint256 distributionId) external onlyCreator(distributionId) {
        Distribution storage distribution = distributions[distributionId];
        ...
        if (distribution.isERC20) {
            IERC20(distribution.token).safeTransfer(distribution.creator, amountLeft);
        } else {
            // Only support an ERC1155 token at id = 0
            IERC1155(distribution.token).safeTransferFrom(address(this), distribution.creator, 0, amountLeft, "");
        }
     ...
     */
    function isWhitelisted(uint256 distributionId, address wallet, bytes32[] calldata merkleProof)
        external
        view
        returns (bool)
    {
        return MerkleProof.verify(
            merkleProof, distributions[distributionId].merkleRoot, keccak256(abi.encodePacked(wallet))
        );
    }
     ...
     */
    function getDistributionsCountByCreator(address creator) external view returns (uint256) {
     ...
     * @return The distribution count
     */
    function getDistributionsCountByToken(address token) external view returns (uint256) {
     ...
     */
    function getAllDistributionIdsByCreator(address creator) external view returns (uint256[] memory) {
     ...
     * @return An array of distribution IDs created by the address.
     */
    function getAllDistributionIdsByToken(address token) external view returns (uint256[] memory) {
    ...
    function _getDistributionOutput(uint256 distributionId) private view returns (DistributionOutput memory) {
     ...
     */
    function getDistributionsByCreator(address creator, uint256 startIndex, uint256 limit)
        external
        view
     ...
     * @return data An array of Distribution structs within the specified range.
     */
    function getDistributionsByToken(address token, uint256 startIndex, uint256 limit)
        external
        view
        returns (uint256[] memory ids, DistributionOutput[] memory data)
    {
