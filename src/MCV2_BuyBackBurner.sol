...
contract MCV2_BuyBackBurner is Ownable {
    ...
    IMintClubBond public constant V1_BOND = IMintClubBond(0x8BBac0C7583Cc146244a18863E708bFFbbF19975);
    address public constant MINT = address(0x1f3Af095CDa17d63cad238358837321e95FC5915);
    address public constant CREATOR = address(0x9f3C60dC06f66b3e0ea1Eb05866F9c1A74d43D67);
    address public constant MINTDAO = address(0x558810B46101DE82b579DD1950E9C717dCc28338);
    address public constant GRANT = address(0x58764cE77f0140F9678bA6dED9D9697c979F4E0f);
    address public OP_FUND_ADDRESS = address(0x5e74f8CC57a3A2d9718Cc98eD7f60D72b0159a14);
    ...
    event PurchasePremium(
        string key, uint256 mintTokenAmount, uint256 creatorAmount, address indexed purchaser, uint256 timestamp
    );
    event BuyBackGrant(uint256 mintTokenAmount, uint256 grantAmount, uint256 timestamp);
    event BuyBackBurnMintDao(uint256 mintTokenAmount, uint256 mintDaoAmount, uint256 timestamp);
     ...
     */
    function setPremiumPrice(string calldata key, uint256 price) external onlyOwner {
     ...
     */
    function purchasePremium(string calldata key, address purchaser, uint256 maxMintTokenToSpend)
        external
        returns (uint256 burned)
    {
        ...
        uint256 mintTokenRequired = estimateReserveAmountV1(CREATOR, creatorPrice);
        ...
        emit PurchasePremium(key, mintTokenRequired, creatorPrice, purchaser, block.timestamp);
    }
     ...
     */
    function buyBackGrant(uint256 mintTokenAmount, uint256 minGrantToBuyBack) external returns (uint256 purchased) {
     ...
     * @param mintTokenAmount The amount of MINT tokens to spend on the buy back
     */
    function buyBackBurnMintDao(uint256 mintTokenAmount, uint256 minMintDaoToBurn) external returns (uint256 burned) {
        if (mintTokenAmount == 0) revert MCV2_BuyBackBurner__InvalidAmount();
     ...
     */
    function estimateReserveAmountV1(address tokenAddress, uint256 tokensToBuy)
        public
        view
        returns (uint256 reserveRequired)
    {
        IERC20 token = IERC20(tokenAddress);

        uint256 currentSupply = token.totalSupply();
        uint256 newTokenSupply = currentSupply + tokensToBuy;
        reserveRequired = (newTokenSupply ** 2 - currentSupply ** 2) / (2 * 1e18);
     ...
     */
    function estimateTokenAmountV1(address tokenAddress, uint256 mintTokenAmount)
        public
        view
     ...
     * @param mintTokenAmount The amount of MINT tokens to spend on the buy back
     */
    function _buyBack(address tokenAddress, uint256 mintTokenAmount) private returns (uint256 purchasedAmount) {
     ...
     */
    function getBurnedStats() external view returns (uint256 totalCreatorBurned, uint256 totalMintDaoBurned) {
